...
import { Users, BookOpen, Sun, Cloud, Moon, ChevronRight as ChevronRightIcon } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const students = storageService.getAlunos(alunos);
      ...
      <div className="space-y-8 pb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {professorTurmas.map((turma) => {
            const studentCount = allStudents.filter(s => s.turmaId === turma.id).length;
            return (
              <div 
                key={turma.id}
              ...
              >
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{studentCount} Alunos Matriculados</p>
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-dashed border-slate-100 flex items-center justify-between">
                  ...
                  <ChevronRightIcon className="text-slate-300 group-hover:translate-x-2 transition-transform" size={20} />
                </div>
              </div>
            );
          })}
